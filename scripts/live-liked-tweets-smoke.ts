...
const canaryUserId = process.env.TWITTER_LIKES_CANARY_USER_ID;
...
const main = async () => {
  if (!canaryUserId) {
    throw new Error(
      "TWITTER_LIKES_CANARY_USER_ID is required for the liked tweets live smoke.",
    );
  }
  ...
  const result = await Effect.runPromise(
    Effect.gen(function* () {
      ...
      const items = yield* Stream.runCollect(
        tweets.getLikedTweets(canaryUserId, { limit: 2 }),
      );

      if (items.length === 0) {
        throw new Error("Liked tweets returned no tweets.");
      }
